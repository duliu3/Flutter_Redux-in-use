import 'dart:async';
import 'package:car_flutter_demo/localization/default_localizations.dart';
import 'package:flutter/foundation.dart';
import 'package:flutter/material.dart';

class CustomLocationsDelegate extends LocalizationsDelegate<GSYLocalizations> {

  CustomLocationsDelegate();

  @override
  bool isSupported(Locale locale) {
    ///支持中文和英语
    return true;
  }

  ///根据locale，创建一个对象用于提供当前locale下的文本显示
  @override
  Future<GSYLocalizations> load(Locale locale) {
    return SynchronousFuture<GSYLocalizations>(GSYLocalizations(locale));
  }

  @override
  bool shouldReload(LocalizationsDelegate<GSYLocalizations> old) {
    return false;
  }

  ///全局静态的代理
  static LocalizationsDelegate<GSYLocalizations> delegate = CustomLocationsDelegate();
}
